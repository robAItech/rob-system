"""
API versioning for the Enterprise Unified Gateway.

Združitev nekdanjega samostojnega modula ``api_versioning`` v gateway:
- ``VersionManager`` — validacija SemVer in upravljanje podprtih/opuščenih različic.
- ``VersionValidationMiddleware`` — uveljavlja ``Accept-Version`` glavo na
  versioniranih poteh (privzeto samo ``/version`` in ``/protected``), da ostale
  gateway poti (routing, webhooks) ostanejo brez prisilnega versioninga.
"""
...
class VersionValidationMiddleware(BaseHTTPMiddleware):
    """
    Middleware that enforces ``Accept-Version`` on a scoped set of versioned paths.

    V združenem gateway-u ne moremo globalno zahtevati ``Accept-Version`` (to bi
    razbilo routing/webhook poti), zato validiramo samo eksplicitno versionirane
    poti (privzeto ``/version`` in ``/protected``).
    """

    def __init__(self, app, version_manager: VersionManager, protected_paths: Optional[set] = None):
        super().__init__(app)
        self.version_manager = version_manager
        self.protected_paths = protected_paths or {"/version", "/protected"}

    async def dispatch(self, request: Request, call_next):
        """
        Process request and validate version header for protected paths only.
        """
        if request.url.path not in self.protected_paths:
            return await call_next(request)

        version_header = request.headers.get("Accept-Version")
        is_valid, error_message = self.version_manager.validate_version_header(version_header)
