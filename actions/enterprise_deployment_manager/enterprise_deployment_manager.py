...
class DeploymentManager:
    ...
    def get_services(self) -> Dict[str, int]:
        ...
        fixed_ports = {
            "enterprise_api_gateway": 8000,
            "enterprise_nexus_command_deck": 8010,
            "enterprise_observability_metrics": 9090
        }
        current_port = 8001
    ...
    def update_gateway_routes(self) -> bool:
        services = self.get_services()
        gateway_main = self.actions_dir / "enterprise_api_gateway" / "main.py"
        ...
        for name, port in services.items():
            if name == "enterprise_api_gateway":
                continue
            ...
            req_auth = "True" if name in ["enterprise_audit_trail", "enterprise_cache_layer"] else "False"
