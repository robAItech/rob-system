"""Rate-limit algoritmi (strategije) za actions/rate_limiter.

Po arhitekturni konsolidaciji ``token_bucket`` ni več samostojen Action modul —
algoritem vedra z žetoni postane ena od strategij ``rate_limiter``. Skozi
``RateLimiter(config.strategy="token_bucket")`` se izbere bodisi privzeta
sliding-window strategija bodisi token-bucket (glej ``rate_limiter.py``).
"""
...
class TokenBucket:
    ...
    def tokens(self) -> float:
        """Trenutno število žetonov (po refillu) — za poročanje ``remaining``."""
        with self._lock:
            self._refill()
            return self._tokens
