        ...
        if (await f.exists()) {
          ...
          for (const line of lines) {
            try {
              ...
              let s = (ev === 'fleet-memory' ? 'MEMORY' : ev === 'fleet-claim' ? 'CLAIM' : ev === 'fleet-result' ? 'RESULT' : ev === 'daemon-task' ? 'TASK' : ev === 'rsi-run' ? 'RSI' : ev === 'build' ? 'BUILD' : ev === 'agent' ? 'AGENT' : ev.toUpperCase().slice(0, 6));
              let c = 's-run';
              ...
              if (st.size > offset) {
                ...
                for (const line of lines) {
                  ...
                  try {
                    ...
                    const ev = String(d.event || '');
                    const s = (ev === 'fleet-memory' ? 'MEMORY' : ev === 'fleet-claim' ? 'CLAIM' : ev === 'fleet-result' ? 'RESULT' : ev === 'daemon-task' ? 'TASK' : ev === 'rsi-run' ? 'RSI' : ev === 'build' ? 'BUILD' : ev === 'agent' ? 'AGENT' : ev.toUpperCase().slice(0, 6));
                    let c = 's-run';
        ...
        if (await ef.exists()) {
          ...
          for (const line of lines) {
            try {
              ...
              if (ev !== 'daemon-task' && ev !== 'daemon-tick' && ev !== 'fleet-claim' && ev !== 'fleet-result' && ev !== 'fleet-memory') continue;
              const ts = new Date(d.ts * 1000);
              ...
              const s = ev === 'fleet-memory' ? 'MEMORY' : ev === 'fleet-claim' ? 'CLAIM' : ev === 'fleet-result' ? 'RESULT' : ev === 'daemon-task' ? 'TASK' : 'TICK';
