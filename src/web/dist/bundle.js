...
var fetchFleetSecurity = () => getJSON("/api/fleet-security");
function ageAgo(ts) {
  if (!ts)
    return "—";
...
// src/web/components/fleet_security.ts
var SEV_LABEL = { critical: "kritična", high: "visoka", medium: "srednja", low: "nizka" };
function esc(s) {
...
function craIcon(status) {
  if (status === "compliant")
    return '<span class="s-ok">✓</span>';
  if (status === "non_compliant")
    return '<span class="s-crit">✗</span>';
  if (status === "partial")
    return '<span class="s-warn">~</span>';
  return '<span class="s-run">—</span>';
}
function sevBadge(sev) {
  const cls = sev === "critical" ? "sev-crit" : sev === "high" ? "sev-warn" : "sev-med";
  return `<span class="sev-badge ${cls}">${esc(SEV_LABEL[sev] || sev)}</span>`;
}
function renderFleetSecurity(el, d) {
  if (!d || d.ok === false) {
  ...
  const sev = findings_by_severity;
...
ready(() => {
  ...
  const startApp = () => {
    ...
    const loadFleetSecurity = () => {
      if (!fleetSecEl)
        return;
      setPanelState(fleetSecEl, "loading");
      fetchFleetSecurity().then((d) => {
        if (isSessionExpired())
          return;
        if (!d?.ok) {
          setPanelState(fleetSecEl, "error", d?.error || "Fleet security ni dosegljiv", loadFleetSecurity);
          return;
        }
        ...
        if (!isSessionExpired())
          setPanelState(fleetSecEl, "error", "Fleet security ni dosegljiv", loadFleetSecurity);
      });
    };
    loadFleetSecurity();
